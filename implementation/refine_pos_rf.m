function [pos, max_response]=refine_pos_rf(im,im2, pos, svm_struct, app_model, config)
...
[feat, pos_samples, ~, weights]=det_samples(im, im2,pos, window_sz, config.detc);
...
if max_response>1.2*config.max_response && max(scores)>0
